...
export default function ChessRemote({props, socket, room}) {

	const [winner, setWinner] = useState(0)

	const navigate = useNavigate()

    if (!props.myProfile.room)
        navigate('/play')
	...
	return (
		<div className="w-100 h-100 d-flex flex-column">
			...
			<div className="d-flex justify-content-center align-items-center w-100 h-100 position-relative">
				...
				<ChessCanvasRemote setWinner={setWinner} socket={socket} />}
			</div>
		</div>
	)
...
function ChessCanvasRemote({setWinner, socket}) {}