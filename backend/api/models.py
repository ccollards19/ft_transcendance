...
class user(AbstractUser):
    ...
    # queue = models.ManyToManyField('self', blank=True, symmetrical=True, related_name='queue')
    # game = models.CharField(choices=GAME)
    # match = models.ForeignKey("match", null=True, on_delete=models.SET_NULL)
    # spectate = models.BooleanField(default=False)
    # gameType = models.BooleanField(default=False)
    # device = models.CharField(choices=DEVICE) 
    # scope  = models.CharField(choices=SCOPE)
